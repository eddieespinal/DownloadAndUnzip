...
- (void)updateProgressWithFileIndex:(uLong)myCurrentFileIndex TotalFiles:(uLong)totalfile
{
    if( _delegate && [_delegate respondsToSelector:@selector(ZipProgress:total:)] )
        [_delegate ZipProgress:myCurrentFileIndex total:totalfile];
}
